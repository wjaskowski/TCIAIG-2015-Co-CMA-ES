...
public final class OthelloLeague {
	...
	public static String toOthelloLeagueFormat(NTuples ntuples) {
		...
		for (NTuple tuple : ntuples.getMain()) {
			...
			for (int[] locations : symmetrical) {
				...
				for (int loc : locations) {
					normalLocations.add(BoardUtils.marginPosToPos(loc, OthelloBoard.SIZE.size()));
				}
	...
	public static void saveInOthelloLeagueFormat(NTuples ntuples, File file) {
		try {
			FileUtils.writeStringToFile(file, toOthelloLeagueFormat(ntuples));
		} catch (IOException e) {
			throw new RuntimeException(e.getMessage(), e);
		}
	}
